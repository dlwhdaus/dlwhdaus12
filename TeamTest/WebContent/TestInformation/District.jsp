		...
		<section id="container">
			...
			<section class="contents">
				...
				<div class="benefit">
					...
					<script>
						$(function(){
							...
							$("#select").change(function(){
								...
								$("#deleteDistrict").val($("#select option:selected").text());
					...
					<h3 style="">구역 등록하기</h3>
					<div>
						<form action="<c:url value='/TestInformation/District.do'/>" method="post">
							지역코드:<input type="text" name="postCode" style="width: 50px" maxlength="2">
						      구역명:<input type="text" name="districtName" style="width: 50px">
					...
					<div>
						<h3 style="">구역 삭제하기</h3>
						<form action="<c:url value='/TestInformation/District.do'/>" method="post">
						      구역명:<input type="text" name="deleteDistrict" style="width: 50px" id="deleteDistrict">
					...
					<h2>구역목록 보기</h2>
					<div>
						<form action="<c:url value='/TestInformation/District.do'/>" method="post" id="districtForm">
							
							
							<select name="pcdt" id="districtSelect">
								  <option>지역 선택하기</option>	
								  <c:forEach items="${postList}" var="post" varStatus="loop">
								  	<option value="${loop.count }">${post.postName } 구역보기</option>
								  </c:forEach>
					...
					<%-- 
							...
							<select name="pcdt" id="select">
								  
								  <c:forEach items="${pcdtdto}" var="pcdtdto" varStatus="loop">
								  	<option value="${pcdtdto.districtName }">${pcdtdto.districtName }</option>
								  </c:forEach>
